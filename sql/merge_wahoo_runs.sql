-- ============================================================================
-- merge_wahoo_runs: mismo rol que merge_strava_runs/merge_polar_runs (ver
-- merge_strava_runs.sql para el porqué de la transacción con FOR UPDATE), acá
-- con el dedupe por wahooId en vez de stravaId/polarId.
--
...
CREATE OR REPLACE FUNCTION public.merge_wahoo_runs(
  p_user_id uuid,
  p_new_runs jsonb,
...
BEGIN
  ...
  LOOP
    ...
    FOR v_idx IN 0 .. jsonb_array_length(v_runs) - 1 LOOP
      IF (v_runs -> v_idx -> 'wahooId') = (v_run -> 'wahooId') THEN
        v_found_idx := v_idx;
        EXIT;
      END IF;
    END LOOP;
